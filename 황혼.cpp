...
int n,m,k;
vector<pll> v[400010];
pair<pii,ll> road[400010];
int prev_node[400010];
ll dist[400010];
int no[400010];
int main()
{
    ...
    for(int i=1;i<=m;i++)
    {
        if(prev_node[road[i].fi.se]==road[i].fi.fi || prev_node[road[i].fi.se]==-1)
            v[road[i].fi.fi+n].push_back({road[i].fi.se+n,road[i].se});
        else
            v[road[i].fi.fi+n].push_back({road[i].fi.se,road[i].se});
        if(prev_node[road[i].fi.se]==-1)
            v[road[i].fi.fi].push_back({road[i].fi.se+n,road[i].se});
        else
            v[road[i].fi.fi].push_back({road[i].fi.se,road[i].se}); 
    }
    fill(dist,dist+2*n+1,1e16);
    priority_queue<pll,vector<pll>,greater<pll>> pq;
    int st=1;
    if(prev_node[st]==-1) st+=n;
    pq.push({0,st});
    while(!pq.empty())
    {
        auto a=pq.top();
        pq.pop();
        if(dist[a.se]<=a.fi) continue;
        dist[a.se]=a.fi;
        for(auto i:v[a.se])
        {
            int nx=i.fi;
    ...
    for(int i=1;i<=n;i++)
        cout<<(min(dist[i],dist[i+n])==1e16?-1:min(dist[i],dist[i+n]))<<" ";
}
