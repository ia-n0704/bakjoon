...
int main()
{
    ...
    while(l<=r)
    {
        ...
        for(int i=1;i<=n;i++)
        {
            while(arr[i]-arr[temp+1]>=mid) temp++;
            idx[i]=temp;
        }
        int resa=0;
        for(int i=1;i<=n;i++)
        {
            resa=max(resa,i-idx[i]);
            dp[i]=max(dp[i-1],dp[idx[i]]+1);
        }
        //cout<<mid<<" | "<<resa<<" "<<n-dp[n]<<" "<<idx[n]<<'\n';
        if(resa>=(n+1)/2+1) r=mid-1;
        else if(n-dp[n]<=(n-2)/2) l=mid+1;
        else
        {
            if((s=="Alice" && n%2==0) || (s=="Bob" && n%2==1))
                r=mid-1;
            else l=mid+1;
        }
    }
