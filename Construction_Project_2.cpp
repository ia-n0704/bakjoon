...
ll l,k;
vector<pll> v[200010];
ll dist_s[200010],dist_e[200010];
int main()
{
    fastio;
    cin>>n>>m>>s>>e>>l>>k;
    ...
    while(!pq.empty())
    {
        ...
        ll co=a.fi;
        //cout<<cur<<" "<<co<<'\n';
        if(dist_s[cur]<co) continue;
        dist_s[cur]=co;
        for(auto i:v[cur])
        {
            if(co+i.se<dist_s[i.fi])
            {
                dist_s[i.fi]=co+i.se;
                pq.push({co+i.se,i.fi});
            }
        }
    }
    ...
    while(!pq.empty())
    {
        ...
        ll cur=a.se;
        ll co=a.fi;
        if(dist_e[cur]<co) continue;
        dist_e[cur]=co;
        for(auto i:v[cur])
        {
            if(co+i.se<dist_e[i.fi])
            {
                dist_e[i.fi]=co+i.se;
                pq.push({co+i.se,i.fi});
            }
    ...
    {
        sort(dist_s+1,dist_s+n+1);
        ll res=0ll;
        for(int i=1;i<=n;i++)
        {
            ll num=dist_e[i]+l;
            //cout<<dist_e[i]<<' ';
            ll idx=upper_bound(dist_s+1,dist_s+n+1,k-num)-(dist_s+1);
            //cout<<idx<<'\n';
            res+=idx;
        }
        cout<<res;
