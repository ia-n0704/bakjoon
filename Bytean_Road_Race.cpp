...
int main()
{
    ...
    for(int i=1;i<=m;i++)
    {
        int a,b;
        cin>>a>>b;
        if(arr[a].fi>arr[b].fi || arr[a].se<arr[b].se) swap(a,b);
        v[a].push_back(b);
        indeg[b]++;
        indeg2[b]++;
    }
    ...
    priority_queue<pair<pii,int>,vector<pair<pii,int>>,greater<pair<pii,int>>> pq2;
    for(int i=1;i<=n;i++)
    ...
    while(k--)
    {
        int a,b;
        cin>>a>>b;
        bool f1=idx1[a]>idx1[b];
        bool f2=idx2[a]>idx2[b];
        if(f1==f2) cout<<"TAK\n";
        else cout<<"NIE\n";
    }
}
