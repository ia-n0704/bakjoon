...
bool condition(int x,int y,int sz)
{
    if(x+sz*3-1>=n || y+sz*3-1>=m) return 0;
    return 1;
}
int main()
{
    ...
    for(int k=1;k<=20;k++)
    {
        for(int i=0;i<n;i++)
        {
            for(int j=0;j<m;j++)
            {
                if(!condition(i,j,k)) continue;
                
                for(int x=0;x<n;x++)
                    for(int y=0;y<m;y++)
                        t[x][y]='.';
                int cnt=0;
                for(int x=i;x<i+k*3;x++)
                {
                    int l=j+k*3;
                    if(cnt/k==1) l=j+k;
                    for(int y=j;y<l;y++)
                ...
                for(int x=0;x<n;x++)
                {
                    for(int y=0;y<m;y++)
                    {
                        if(s[x][y]=='#' && t[x][y]=='.') res+=b;
                        else if(s[x][y]=='.' && t[x][y]=='#') res+=a;
                    }
                }
                ans1=min(ans1,res);
            }
