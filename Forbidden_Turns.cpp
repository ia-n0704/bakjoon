...
int main()
{
    ...
    while(!pq.empty())
    {
        ...
        for(auto i:v[cur])
        {
            ll nx=i.fi;
            ll nx_co=co+i.se;
            //cout<<" | "<<prev<<" "<<cur<<" "<<nx<<s.count({prev,{cur,nx}})<<'\n';
            if(s.count({prev,{cur,nx}})) continue;
            if(!ss.count({prev,{cur,nx}}))
            {
                pq.push({nx_co,{nx,cur}});
                ss.insert({prev,{cur,nx}});
            }
            if(nx_co<dist[nx]) dist[nx]=nx_co;       
        }
    }
    if(dist[en]==1e18) cout<<-1;
    else cout<<dist[en];
}
