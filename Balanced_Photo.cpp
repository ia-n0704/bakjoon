...
int main()
{
    ...
    for(int i=1;i<=n;i++)
    {
        int temp=segA.query(1,0,100000,arr[i],100000);
        l[i]=temp;
        segA.update(1,0,100000,arr[i],1);
    }
    for(int i=n;i>=1;i--)
    {
        int temp=segB.query(1,0,100000,arr[i],100000);
        r[i]=temp;
        segB.update(1,0,100000,arr[i],1);
    }
    int cnt=0;
    for(int i=1;i<=n;i++)
        if(max(l[i],r[i])>min(l[i],r[i])*2) cnt++;
    cout<<cnt;
}
