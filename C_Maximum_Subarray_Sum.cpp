...
void sol()
{
    ...
    for(int i=1;i<=n;i++) l[i]=r[i]=arr[i];
    for(int i=1;i<=n;i++) l[i]=max(l[i],l[i]+l[i-1]);
    for(int i=n-1;i>=1;i--) r[i]=max(r[i],r[i]+r[i+1]);
    ll res=0;
    for(int i=1;i<=n;i++)
    {
        res=max(res,max(0ll,l[i-1])+max(0ll,r[i+1])+arr[i]);
    }
    if(res!=k)
    {
        cout<<"No\n";
