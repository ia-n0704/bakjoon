...
int main()
{
    ...
    for(int i=1;i<=q;i++)
    {
        ll a,b;
        cin>>a>>b;
        ll x=lower_bound(arr+1,arr+n+1,a)-arr;
        ll y=upper_bound(arr+1,arr+n+1,b)-arr;
        y--;
        ll mid=(x+y)/2;
        ll mn=(mid-x)*arr[mid]-sum[mid-1]+sum[x-1]+sum[y]-sum[mid]-(y-mid)*arr[mid];
        ll mx=(y-x+1)*arr[y]-(sum[y]-sum[x-1]);
        mx=max(mx,sum[y]-sum[x-1]-(y-x+1)*arr[x]);
        cout<<mx-mn<<'\n';
    }
}
