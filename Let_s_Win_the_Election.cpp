...
int main()
{
    ...
    for(int i=0;i<=n;i++)
    {
        for(int j=i+1;j<=n;j++)
            brr[j-i]=arr[j].fi;
        sort(brr+1,brr+n-i+1);
        for(int j=i+1;j<=n;j++)
            sum[i][j-i]=sum[i][j-i-1]+brr[j-i];
    }
    double res=1e9;
    for(int x=0;x<=n;x++)
    {
        ...
        for(int i=1;i<=n;i++)
        {
            for(int j=0;j<=n;j++)
            {
                ...
                dp[i][j+1]=min(dp[i][j+1],dp[i-1][j]+arr[i].se/(double)(j+1));
                dp[i][j]=min(dp[i][j],dp[i-1][j]+arr[i].fi/(double)(x+1));
                //cout<<i<<" "<<j<<" | "<<dp[i][j]<<'\n';
            }
        }
        for(int i=0;i<=k;i++)
        {
            res=min(res,dp[i][x]+sum[i][k-i]/(double)(x+1));
            //cout<<"res : "<<res<<'\n';
        }
    }
